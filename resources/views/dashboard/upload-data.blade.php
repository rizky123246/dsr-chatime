...
@section('title', 'Upload Data Pembayaran')
...
<div class="dashboard-content">
    ...
    <div class="upload-card">
        <h2>Upload Data Pembayaran</h2>
        <p class="subtitle">Upload file CSV hasil ekspor dari sistem Point of Sale</p>

        <div class="input-group">
            <label class="label-file">File CSV</label>
            <div class="file-selector" onclick="document.getElementById('csvFile').click()">
                <strong>Choose File</strong> &nbsp; <span id="fileName">No file chosen</span>
            </div>
            <input type="file" id="csvFile" accept=".csv" onchange="handleFileSelect(event)">
        </div>

        <div class="format-hint">
            Format file: CSV dengan kolom Transaction ID, Date, Time, Product Name, Quantity, Price, Discount, Total, Payment Method
        </div>

        <div class="action-group">
            <button class="btn btn-primary" onclick="uploadFile()">
                <span class="icon">↑</span> Upload & Proses
            </button>
            <button class="btn btn-outline" onclick="useSampleData()">
                <span class="icon">☰</span> Gunakan Data Contoh
            </button>
            <button class="btn btn-outline" onclick="downloadTemplate()">
                <span class="icon">↓</span> Download Template CSV
            </button>
        </div>

        <!-- Progress Section -->
        <div class="progress-section" id="progressSection">
            <div class="progress-text">Mengupload dan memproses file...</div>
            <div class="progress-bar">
                <div class="progress-fill" id="progressFill"></div>
            </div>
            <div class="progress-status" id="progressStatus">Memvalidasi format file...</div>
        </div>

        <!-- Success Section -->
        <div class="success-section" id="successSection">
            <div class="success-title">✅ Data berhasil diupload!</div>
            <div class="success-details" id="successDetails">
                Total 156 transaksi berhasil diproses dan ditambahkan ke database.
            </div>
        </div>
    </div>

    <!-- Upload Data Penjualan Section -->
    <div class="upload-card" style="margin-top: 30px;">
        <h2>Upload Data Penjualan</h2>
        <p class="subtitle">Upload file CSV hasil ekspor dari sistem Point of Sale untuk data penjualan</p>

        <div class="input-group">
            <label class="label-file">File CSV Penjualan</label>
            <div class="file-selector" onclick="document.getElementById('salesCsvFile').click()">
                <strong>Choose File</strong> &nbsp; <span id="salesFileName">No file chosen</span>
            </div>
            <input type="file" id="salesCsvFile" accept=".csv" onchange="handleSalesFileSelect(event)">
        </div>

        <div class="format-hint">
            Format file: CSV dengan kolom Transaction ID, Date, Time, Product Name, Quantity, Price, Discount, Total, Customer Name, Payment Method
        </div>

        <div class="action-group">
            <button class="btn btn-primary" onclick="uploadSalesFile()">
                <span class="icon">↑</span> Upload & Proses Penjualan
            </button>
            <button class="btn btn-outline" onclick="useSalesSampleData()">
                <span class="icon">☰</span> Gunakan Data Contoh Penjualan
            </button>
            <button class="btn btn-outline" onclick="downloadSalesTemplate()">
                <span class="icon">↓</span> Download Template Penjualan CSV
            </button>
        </div>

        <!-- Sales Progress Section -->
        <div class="progress-section" id="salesProgressSection">
            <div class="progress-text">Mengupload dan memproses file penjualan...</div>
            <div class="progress-bar">
                <div class="progress-fill" id="salesProgressFill"></div>
            </div>
            <div class="progress-status" id="salesProgressStatus">Memvalidasi format file penjualan...</div>
        </div>

        <!-- Sales Success Section -->
        <div class="success-section" id="salesSuccessSection">
            <div class="success-title">✅ Data penjualan berhasil diupload!</div>
            <div class="success-details" id="salesSuccessDetails">
                Total 156 transaksi penjualan berhasil diproses dan ditambahkan ke database.
            </div>
        </div>
    </div>
    </div>
